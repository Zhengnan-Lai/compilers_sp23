...
public class SerialVersionUID implements Serializable {
    ...
    public static long generate(Class<?> clazz) {
        try {
            ...
            byte[] className = clazz.getName().getBytes(StandardCharsets.UTF_8);
            md.update(className);
            ...
            for (Field field : fields) {
                ...
                md.update(field.getName().getBytes(StandardCharsets.UTF_8));

                // Add field type (if primitive) to hash.
                Class<?> fieldType = field.getClass();
                if (fieldType.isPrimitive()) {
                    md.update(fieldType.getName().getBytes(StandardCharsets.UTF_8));
                }
            }
