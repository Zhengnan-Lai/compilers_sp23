...
public class IRSimulator {
    ...
    private final ExprStack exprStack;
    private final BufferedReader inReader;

    private final Set<String> libraryFunctions;
    ...
    private void initialize() {
        ...
        for (IRData dataWrapper : compUnit.dataMap().values()) {
            ...
            long globalVariableStart = calloc(size * 8L);
                ...
                case "_Ireadln_ai":
                    {
                        ...
                        int len = line.length();
                        long ptr = malloc((long) (len + 1) * ws);
                        store(ptr, len);
                        for (int i = 0; i < len; ++i) store(ptr + (long) (i + 1) * ws, line.charAt(i));
                        ret.add(ptr + ws);
                        break;
                    }
                case "_Igetchar_i":
                ...
                case "_IparseInt_t2ibai":
                    {
                        ...
                        long ptr = args[0], size = read(ptr - ws);
                        for (int i = 0; i < size; ++i) buf.append((char) read(ptr + (long) i * ws));
